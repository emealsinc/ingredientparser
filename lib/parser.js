(function (module) {
    ...
    var packageTypes = Defaults.packageTypes;
    ...
    var Parser = function (opts) {
        ...
        var packageTypes = options.packageTypes || Defaults.packageTypes;
        ...
        var packagesKeys = Object.keys(packageTypes);
        var packagesTable = {};
        packagesKeys.forEach(function (key) {
            expandedPackages = expandedPackages.concat(packageTypes[key]);
            packageTypes[key].forEach(function(alt) {
                packagesTable[alt] = key;
            })
        })
        ...
        this.expandedPackages = expandedPackages;
    ...
    Parser.prototype.isTypeOfPackage = function (value) {
        value = value.replace(/[^a-zA-Z]/g, "");

        if (this.packageTypes[value.toLowerCase()] || this.expandedPackages.indexOf(value) > -1) {
            return true;
        }
    ...
    Parser.prototype.packageExpander = function (packageName) {
        if (Defaults.singularizeFixes.includes(packageName)) {
            return properCase(packageName.toLowerCase());
        } else {
            var val = inflect.singularize(packageName);

            val = properCase(this.packagesTable[val.toLowerCase()] || this.packagesTable[val] || val);
    ...
    Parser.prototype.getPackageType = function (from) {
        var _this = this;
        var packages = [];
        ...
        from.forEach(function(val, idx) {
            ...
            if (_this.isTypeOfPackage(val)) {
                val = _this.packageExpander(val);
                packages.push(val);
                from.splice(idx, 1);
            }
        });
        ...
        if (packages.length > 0) {
            var packageName = packages.join(" ").replace(/[^A-Za-z0-9]/, '');
            return packageName;
        } else {
            return false;
        }
    ...
    Parser.prototype.parse = function (source) {
        ...
        if (val = this.getPackageType(parts)) {
            ing.package = val;
        }
